...
const ServicePlans: React.FC = () => {
  const plans = [{
    ...
    features: ['Gestão de até 50 palavras-chave', 'Configuração inicial de campanha', 'Relatório mensal de desempenho', 'Otimização mensal de anúncios', 'Suporte por e-mail'],
    isPopular: false
  }, {
    name: 'Avançado',
    price: 'R$ 997',
    ...
    features: ['Gestão de palavras-chave ilimitadas', 'Estratégia completa de funil de vendas', 'Dashboard em tempo real', 'Otimização contínua de campanhas', 'Integrações com CRM e plataformas', 'Relatórios personalizados', 'Suporte prioritário 7 dias por semana'],
    isPopular: false
  }];
  ...
  return <section className="py-16 bg-brand-gray-50">
      <div className="container">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className="text-3xl font-bold text-brand-blue-800 mb-4">Pronto para começar a anunciar com estratégia?</h2>
          <p className="text-lg text-gray-600">Escolha o plano ideal para maximizar seus resultados no Google Ads</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {plans.map((plan, index) => <div key={index} className={`bg-white rounded-xl shadow-lg overflow-hidden transition-transform hover:transform hover:scale-105 ${plan.isPopular ? 'border-2 border-brand-blue' : 'border border-gray-100'}`}>
              {plan.isPopular && <div className="bg-brand-blue text-white py-2 px-4 text-center">
